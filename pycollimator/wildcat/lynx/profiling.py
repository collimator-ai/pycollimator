...
class Profiler:
    ...
    _min_profiles = {}
    _max_profiles = {}
    _logger = logging.getLogger("wildcat_profiler")
    _disable_profiling = os.environ.get("WILDCAT_DISABLE_PROFILING", "0") == "1"
    # JAX JIT profiling must be disabled by default because it breaks various
    # models and advanced scenarios (autodiff, etc.)
    _enable_jaxjit_profiling = os.environ.get("WILDCAT_JAXJIT_PROFILING", "0") == "1"
    _jaxjit_or_abort = os.environ.get("WILDCAT_JAXJIT_OR_ABORT", "0") == "1"
    ...
    def jaxjit_profiledfunc(cls, fn, name=None):
        ...
        if not cls._enable_jaxjit_profiling or not isinstance(
            ...
            if cls._enable_jaxjit_profiling:
                if cls._jaxjit_or_abort:
                    raise RuntimeError(
                        f"WILDCAT_JAXJIT_OR_ABORT=1 but {name} isn't a jitted function"
                    )
                cls._logger.warning(
        ...
        cls._logger.debug(
            "If compilation fails, consider setting WILDCAT_JAXJIT_PROFILING=0"
            + " (or unset it))"
        )
