...
class CompactEV(LeafSystem):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mot_volt_based_enable_state_idx = self.declare_discrete_state(
            default_value=0.0, as_array=False
        )
        self.declare_periodic_discrete_update(
            self._update_mot_volt_enable_state,
            period=dt,
            offset=dt,
            state_index=self.mot_volt_based_enable_state_idx,
        )

        # the app and bpp ZOH.
        self.app_zoh_state_idx = self.declare_discrete_state(
            default_value=0.0, as_array=False
        )
        self.declare_periodic_discrete_update(
            self._update_app_zoh,
            period=dt,
            offset=dt,
            state_index=self.app_zoh_state_idx,
        )

        self.bpp_zoh_state_idx = self.declare_discrete_state(
            default_value=0.0, as_array=False
        )
        self.declare_periodic_discrete_update(
            self._update_bpp_zoh,
            period=dt,
            offset=dt,
            state_index=self.bpp_zoh_state_idx,
        )

        # the veh_spd and coolant_temnp ZOH.
        self.veh_spd_zoh_state_idx = self.declare_discrete_state(
            default_value=0.0, as_array=False
        )
        self.declare_periodic_discrete_update(
            self._update_veh_spd_zoh,
            period=dt,
            offset=dt,
            state_index=self.veh_spd_zoh_state_idx,
        )

        self.coolant_zoh_state_idx = self.declare_discrete_state(
            default_value=0.0, as_array=False
        )
        self.declare_periodic_discrete_update(
            self._update_coolant_zoh,
            period=dt,
            offset=dt,
            state_index=self.coolant_zoh_state_idx,
        )
    ...
    def _update_mot_volt_enable_state(self, time, state, e, **params):
        enable_prev = state.discrete_state[self.mot_volt_based_enable_state_idx]
    ...
    def ode(self, time, state, *inputs, **parameters):
        ...
        mot_volt_based_enable_state = state.continuous_state[
            self.mot_volt_based_enable_state_idx
        ]
        app_zoh_state = state.discrete_state[self.app_zoh_state_idx]
        bpp_zoh_state = state.discrete_state[self.bpp_zoh_state_idx]
        veh_spd_zoh_state = state.discrete_state[self.veh_spd_zoh_state_idx]
        coolant_zoh_state = state.discrete_state[self.coolant_zoh_state_idx]
