...
class CompassGait(LeafSystem):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Extra states for "left leg is stance" and "toe position" for visualization
        self.toe_index = self.declare_discrete_state(shape=(), default_value=0.0)
        self.stance_index = self.declare_discrete_state(shape=(), default_value=True)

        self.declare_continuous_state_output()
        self.declare_discrete_state_output(state_index=0)
        self.declare_discrete_state_output(state_index=1)
    ...
    def reset(self, time, state, *inputs, **parameters):
        ...
        left_leg_is_stance = state.discrete_state[self.stance_index]
        xd = state.discrete_state
        xd[self.stance_index] = jnp.logical_not(left_leg_is_stance)
        xd[self.toe_index] -= 2 * L * jnp.sin(hip_angle / 2)
